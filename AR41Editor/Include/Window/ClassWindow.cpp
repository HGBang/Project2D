...
#include "../GameObject/TestPlayer2D.h"
...
bool CClassWindow::Init()
{
	...
	m_PortalName = CreateWidget<CEditorInput>("PortalInput");
	m_PortalName->SetSize(250.f, 100.f);
	m_PortalName->SetInputType(EImGuiInputType::String);
...
void CClassWindow::ObjectCreateCallback()
{
	...
	else if (m_SelectObjectItem == "TestMonster")
		Obj = Scene->CreateObject<CTestMonster>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "TestPlayer2D")
		Obj = Scene->CreateObject<CTestPlayer2D>(m_SelectObjectItem);

	else if(m_SelectObjectItem == "SelectPlayer")
		Obj = Scene->CreateObject<CSelectPlayer>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "ElderNpc")
		Obj = Scene->CreateObject<CElderNpc>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "ShopNpc")
		Obj = Scene->CreateObject<CShopNpc>(m_SelectObjectItem);

	else if(m_SelectObjectItem == "Rope")
		Obj = Scene->CreateObject<CRope>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "Helena")
		Obj = Scene->CreateObject<CHelena>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "HonTail")
		Obj = Scene->CreateObject<CHonTail>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "HonTailSummon")
		Obj = Scene->CreateObject<CHonTailSummon>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "Skelegon")
		Obj = Scene->CreateObject<CSkelegon>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "DarkWybern")
		Obj = Scene->CreateObject<CDarkWybern>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "HonTailWall")
		Obj = Scene->CreateObject<CHonTailWall>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "HonTailExObj")
		Obj = Scene->CreateObject<CHonTailExObj>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "SkillEffect")
		Obj = Scene->CreateObject<CSkillEffect>(m_SelectObjectItem);

	else if (m_SelectObjectItem == "Portal")
	{
	   Obj = Scene->CreateObject<CPortal>(m_SelectObjectItem);
	   CPortal* Portal = (CPortal*)Obj;
	   Portal->SetPortalName(m_PortalName->GetText());
	}
...
void CClassWindow::ComponentCreateCallback()
{
	...
	//else if (m_SelectComponentItem == "MovementComponent");
	//{
	//	Name = "MovementComponent(MovementComponent)";
	//	NewObjectComponent = (CObjectComponent*)SelectObject->CreateComponent<CMovementComponent>("MovementComponent");
	//}
...
void CClassWindow::ObjectManyCreateButton()
{
	CScene* Scene = CSceneManager::GetInst()->GetScene();
	...
	else if (m_SelectObjectItem == "TestMonster")
	{
		...
		{
			Obj = Scene->CreateObject<CTestMonster>(m_SelectObjectItem);
			Obj->AddRelativePositionX(1.f + 10 * i);
			++m_Loding;
		}
	}
