...
bool CEngine::m_UITextTime = true;
std::string CEngine::m_UIText;
unsigned int CEngine::m_Number;
...
int CEngine::Run()
{
	...
	while (m_Loop)
	{
		...
		else
		{
			...
			m_Number = (int)rand() % 100; // 엔진에서 난수를 계속 갱신 한다 . 
...
void CEngine::Logic()
{
	...
	CTimerManager::GetInst()->Update(DeltaTime);
...
bool CEngine::FirstUpdate(float DeltaTime)
{
	bool Result = CSceneManager::GetInst()->FirstUpdate(DeltaTime);
