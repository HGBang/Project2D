...
struct AnimationState
{
	...
	std::string ChangeAnimationName;

	std::function<bool(void)> AnimationFunc;

	AnimationState(const std::string& Name , const std::string& ChangeName, std::function<bool(void)> Func)
	{
		AnimationName = Name;
		ChangeAnimationName = ChangeName;
		AnimationFunc = Func;
	}
...
private:
	...
	std::unordered_map<std::string, AnimationState> m_mapAnimationState;
...
public:
	...
	template <typename T>
	void SetCurrentStartFunction(const std::string& AnimationName, T* Obj, void(T::* Func)())
	{
		CAnimation2DData* Animation = FindAnimation(AnimationName);
		...
		Animation->SetStartFunction<T>(Obj, Func);	}
...
public:
	void AddAnimationState(const std::string Name, const std::string ChangeName, std::function<bool(void)> AnimationFunc);
