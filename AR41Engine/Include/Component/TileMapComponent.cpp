	...
	int CountY, const Vector2& TileSize)
{
	...
	{
		m_vecTileInfo[i].TypeColor = Vector4(0.f, 1.f, 0.f, 1.f);
...
void CTileMapComponent::DeleteTile(const Vector2& Pos)
{
	CTile* Tile = GetTile(Pos);
...
void CTileMapComponent::DeleteAllTile()
{
	for (int i = 0; i < m_Count; ++i)
	{
		m_vecTile[i]->m_Opacity = 0.f;
	}
}
...
bool CTileMapComponent::Init()
{
	...
	m_TileEndFrame = Vector2(64.f, 64.f);

	m_vecMaterial.clear();

	AddMaterial("DefaultTileMapBack");

	//SetTileMaterial("DefaultTileMapBack");

    //SetTileBackTexture("Default", TEXT("SelectPlayerBack.png"));	

	//SetTileBackTexture("Henesis", TEXT("Henesis.png"));

	//SetTileBackTexture("HenesisFront", TEXT("HenesisFront.png"));

	//SetTileBackTexture("HenesisShop", TEXT("HenesisShop.png"));

	SetTileBackTexture("BasicBack", TEXT("BasicBack.png"));

	//SetTileBackTexture("HenesisClass", TEXT("HenesisChangeClass.png"));

	//SetTileBackTexture("HonTailBack", TEXT("HonTailMapBack.png"));

	//SetTileBackTexture("HonTailFrontBack", TEXT("HonTailFrontMap.png"));
...
void CTileMapComponent::Render()
{
	...
	if (m_TileBackTexture)
	{
		...
		float Width = (float)m_vecMaterial[0]->GetTexture()->GetWidth();
		float Height = (float)m_vecMaterial[0]->GetTexture()->GetHeight();
	...
	if (m_RenderCount > 0 && m_TileMaterial)
	{
		...
		{
			m_TileGridShader->SetShader();
			m_TileGridMesh->RenderInstancing(m_RenderCount);
		}
