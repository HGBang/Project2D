...
bool CSpriteComponent::SetTextureFullPath(const std::string& Name, const TCHAR* FullPath)
{
	...
	m_vecMaterial[0]->SetTexture(0, 0, (int)EShaderBufferType::Pixel, Name, FullPath);
...
bool CSpriteComponent::SetTextureFullPath(const std::string& Name, const std::vector<const TCHAR*>& vecFullPath)
{
	...
	m_vecMaterial[0]->SetTexture(0, 0, (int)EShaderBufferType::Pixel, Name, vecFullPath);
...
bool CSpriteComponent::SetChangeWidthHeight(bool Change)
{
	m_TextureSize = Change;
...
{
	if (m_Animation)
		m_Animation->SetShader();//Set CBuffer
...
void CSpriteComponent::Set2DZOffset(float In2DZOffset)
{
