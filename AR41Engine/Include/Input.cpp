...
#include "Component/CameraComponent.h"
#include "Scene/CameraManager.h"
#include "Resource/Shader/MouseShader.h"
...
bool CInput::Init(HINSTANCE hInst, HWND hWnd)
{
	...
	//m_MouseMaterial->SetTexture(0, 0, (int)EShaderBufferType::Pixel, "Cursor", TEXT("Cursor.png"));	

	m_MouseSize = Vector2(24.f, 28.f);
...
void CInput::Render()
{
	...
	Vector2 MouseRealPos = m_MousePos - Vector2(-10.f, 10.f);

	matScale.Scaling(m_MouseSize);
	matTranslate.Translation(MouseRealPos);
...
void CInput::UpdateMouse(float DeltaTime)
{
	...
	// WorldPosition 을 만들어 준다 . 마우스의 위치에  카메라의 위치정보를 더한다 . 
