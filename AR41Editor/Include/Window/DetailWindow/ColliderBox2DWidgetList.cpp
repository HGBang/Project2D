...
{	
	...
	m_ColliderList->SetSelectCallback<CColliderBox2DWidgetList>(this, &CColliderBox2DWidgetList::ColliderListClickCallBack);
	...
	ColliderButton->SetClickCallback<CColliderBox2DWidgetList>(this, &CColliderBox2DWidgetList::ColliderClickButton);
	...
	ColliderButton->SetClickCallback<CColliderBox2DWidgetList>(this, &CColliderBox2DWidgetList::ColliderCancelButton);
	ColliderButton->SetSizeY(72.f);
...
void CColliderBox2DWidgetList::Render()
{
	...
	if (Box2DUpdate)
	{
		...
		CColliderBox2D* Box = (CColliderBox2D*)Component;
		Box->SetBoxSize(m_StartX->GetFloat(), m_StartY->GetFloat());
...
void CColliderBox2DWidgetList::ColliderListClickCallBack(int Index, const std::string& Item)
{
	...
	{
		if (m_SelectListItem == "ColliderBox2D")
		{
			CColliderBox2D* Box = (CColliderBox2D*)Component;

			m_Name->SetText("Box2D");
			m_Name->ReadOnly(true);

			m_TypeName->SetInputType(EImGuiInputType::String);
			m_TypeName->SetText("AABB");
			m_TypeName->ReadOnly(true);

			Vector2 BoxSize = Box->GetBoxSize();
...
void CColliderBox2DWidgetList::ColliderClickButton()
{
	CSceneComponent* Component = m_DetailWindow->GetSelectComponent();
...
void CColliderBox2DWidgetList::ColliderCancelButton()
{
	if (m_SelectListItem == "ColliderBox2D")
	{
...
void CColliderBox2DWidgetList::Clear()
{
	m_Name->SetText("");
	m_TypeName->SetText("");
...
void CColliderBox2DWidgetList::SetAddColiderList(const std::string& Name)
{
