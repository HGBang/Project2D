...
bool CAnimation2DWindow::Init()
{
	...
	CreateSquenceButton->SetClickCallback<CAnimation2DWindow>(this, &CAnimation2DWindow::CreateAnimation2D);
...
void CAnimation2DWindow::LoadAnimation2DSequenceList()
{
	...
	for (const auto& file : std::filesystem::recursive_directory_iterator(Directory))
	{
		...
		if (strcmp(Ext, ".SQC") != 0 && strcmp(Ext, ".SRC") != 0)
			continue;
...
void CAnimation2DWindow::SequenceSelectCallback(int Index, const std::string& Item)
{
	...
	CSpriteComponentWidgetList* SpriteWidget = (CSpriteComponentWidgetList*)DetailWindow->GetComponentWidgetList("SpriteComponent");
