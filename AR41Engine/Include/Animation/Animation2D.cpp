...
void CAnimation2D::Update(float DeltaTime)
{
	...
	{
		if (m_CurAnimation->GetCurrentFrame() == 0 && m_CurAnimation->m_StartFunction)
			m_CurAnimation->m_StartFunction();
	...
	if (m_CurAnimation)
	{
		std::unordered_map<std::string, AnimationState>::iterator iter = m_mapAnimationState.find(m_CurAnimation->GetName());
		if (iter != m_mapAnimationState.end())
		{
			if ((*iter).second.AnimationFunc())
			{
				ChangeAnimation((*iter).second.ChangeAnimationName);
			}
...
void CAnimation2D::SetShader()
{
	...
	if(m_Owner)
	{
		Buffer->SetImageReverse(m_Owner->GetReverseX());
	}
...
void CAnimation2D::AddAnimationState(const std::string Name, const std::string ChangeName, std::function<bool(void)> AnimationFunc)
{	
	m_mapAnimationState.insert(std::make_pair(Name, AnimationState(Name, ChangeName, AnimationFunc)));
}
