...
CUIDamageNumber::CUIDamageNumber()
{
    m_WidgetTypeName = "UIDamageNumber";
}

CUIDamageNumber::CUIDamageNumber(const CUIDamageNumber& Number) :
    CUIWidget(Number)
{
...
void CUIDamageNumber::SetUIPos(const Vector2& Pos)
{
    m_Pos = Pos;
...
void CUIDamageNumber::SetTexture(CTexture* Texture)
{
    m_TextureInfo.Texture = Texture;
}

bool CUIDamageNumber::SetTexture(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
    if (m_Scene)
    {
...
bool CUIDamageNumber::SetTextureFullPath(const std::string& Name, const TCHAR* FullPath)
{
    if (m_Scene)
    {
...
bool CUIDamageNumber::SetTexture(const std::string& Name, const std::vector<const TCHAR*>& vecFileName,
...
bool CUIDamageNumber::SetTextureFullPath(const std::string& Name, const std::vector<const TCHAR*>& vecFullPath)
{
    if (m_Scene)
    {
...
void CUIDamageNumber::SetImageTint(const Vector4& Tint)
{
    m_TextureInfo.Tint = Tint;
}

void CUIDamageNumber::SetImageTint(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
    m_TextureInfo.Tint = Vector4(r / 255.f, g / 255.f, b / 255.f, a / 255.f);
}

void CUIDamageNumber::AddFrameData(const Vector2& Start, const Vector2& End)
{
...
void CUIDamageNumber::SetPlayTime(float PlayTime)
{
...
void CUIDamageNumber::SetPlayScale(float PlayScale)
{
    m_TextureInfo.PlayScale = PlayScale;
}

void CUIDamageNumber::Start()
{
    CUIWidget::Start();
}

bool CUIDamageNumber::Init()
{
    if (!CUIWidget::Init())
        return false;
...
void CUIDamageNumber::Update(float DeltaTime)
{  
    ...
    if (m_DestroyTime > 0.3f)
    {
        Destroy();        
    }

    m_OpcityTime += DeltaTime;

    if (m_OpcityTime > 0.03f)
    {
        float Opacity = m_StartOpacity / 10.f;
            SetOpacity(Opacity);
            --m_StartOpacity;
            m_OpcityTime = 0.f;
    }
    ...
    CameraWorldPos.y = CameraWorldPos.y + 130.f * DeltaTime;
    m_WorldPos.y = m_WorldPos.y + 130.f * DeltaTime;

    Vector2 RealPos = m_WorldPos - CameraWorldPos;
...
void CUIDamageNumber::PostUpdate(float DeltaTime)
{
    CUIWidget::PostUpdate(DeltaTime);
}

void CUIDamageNumber::Render()
{
...
CUIDamageNumber* CUIDamageNumber::Clone()
{
    return new CUIDamageNumber(*this);
}

void CUIDamageNumber::Save(FILE* File)
{
    CUIWidget::Save(File);
...
void CUIDamageNumber::Load(FILE* File)
{
    CUIWidget::Load(File);
